...
static lv_res_t decoder_info(struct _lv_img_decoder_t * decoder, const void * src, lv_img_header_t * header)
{
    ...
    /*If it's a PNG file in a  C array...*/
        ...
        header->always_zero = 0;
        header->cf = LV_IMG_CF_TRUE_COLOR_ALPHA;       /*Save the color format*/
...
#if 1
    // How on earth did this file ever build??
    // Increase the scope of png_data and png_data_size so they are available
    // through both code paths. Otherwise it fails to build.
    // Then png_data needs to be loaded.
